...
def update_checkout(id_karyawan, tanggal, jam_keluar, lokasi_absensi, jam_kurang, total_jam_kerja):
    try:
        result = connection.execute(
            text("""UPDATE Absensi 
                    SET jam_keluar = :jam_keluar, lokasi_keluar = :lokasi_keluar, jam_kurang = :jam_kurang, 
                        total_jam_kerja = :total_jam_kerja, updated_at = :timestamp_wita
    ...
    try:
        result = connection.execute(
            text("""
                ...
                AND status = 1
            """),
            {'id_absensi': id_absensi, 'timestamp_wita': timestamp_wita}
        )
        connection.commit()
